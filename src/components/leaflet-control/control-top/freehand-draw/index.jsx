...
import turf from "../../../../hooks/turf-freehand/myTurf";

function FreehandsDraw({ map }) {
  useEffect(() => {
    if (!map) return;
    ...
    const customControler = L.Control.extend({
      ...
      _erase: function (latlng, zoom) {
        if (this._data === undefined || this._data === null) {
          ...
          this.setData(
            turf.difference(
              this._data,
              this._getCircleAsPolygon(latlng, zoom, 12)
            )
          );
        }
      },
      ...
      _processStack: function () {
        ...
        if (evt.action === "draw") {
          ...
          this._erase(evt.latlng, evt.zoom);
        }
...
FreehandsDraw.propTypes = {
  map: PropTypes.any,
};

export default FreehandsDraw;
